...
if __name__ == "__main__":
    ...
    long_df = pd.wide_to_long(pooled_df, stubnames=["HTO derived cDNA - ", "mRNA derived cDNA - ", "Pool - "], i="Name", j="Sample Number", sep="")
    ...
    pool_contents_df["Pool"] = pool_contents_df["Pool"].str.replace("Pool", "").str.replace("_", "")
    pool_contents_df = pool_contents_df.drop_duplicates(ignore_index=True).sort_values(by=["Pool"]).reset_index(drop=True)
    pool_contents_df['idx'] = pool_contents_df.groupby('Pool').cumcount()

    print(pool_contents_df.tail(10))
    pivoted_pool_contents_df = pool_contents_df.pivot(index="Pool", columns="idx", values="Bulk RNA seq")
    pivoted_pool_contents_df = pivoted_pool_contents_df.dropna(how="all", axis=1)
    print(pivoted_pool_contents_df)

    pivoted_pool_contents_df.to_csv("../data/sample_matches/sample_matches_cnst_pool_contents.csv", index=True)

    # Just the sample id mapping
    ...
    mapping_df = pd.concat([long_df, non_pooled_df[["Name", "Bulk RNA seq", "mRNA derived cDNA"]].set_index("Name")], axis=0)
    # mapping_df.to_csv("../data/sample_matches/sample_matches_central_nervous_system_tumor.csv", index=True)

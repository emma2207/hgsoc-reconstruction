...
if [ "${RUN_MODE}" == "HPC" ]; then
    nextflow run main_from_genotype.nf -profile slurm -w "${NEXTFLOW_WORK_DIR}" -process.echo -resume -with-trace
else
    nextflow run main_from_genotype.nf -profile local -w "${NEXTFLOW_WORK_DIR}" -process.echo -resume -with-trace
fi

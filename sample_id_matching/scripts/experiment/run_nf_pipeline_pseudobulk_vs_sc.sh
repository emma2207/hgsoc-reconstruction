...
if [ "${RUN_MODE}" == "HPC" ]; then
    nextflow run main_pseudobulk_vs_sc.nf -profile slurm -w "${NEXTFLOW_WORK_DIR}" -process.echo
else
    nextflow run main_pseudobulk_vs_sc.nf -profile local -w "${NEXTFLOW_WORK_DIR}" -process.echo
fi

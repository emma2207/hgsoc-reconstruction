...
if [ "${RUN_MODE}" == "HPC" ]; then
    nextflow run main_missing_samples.nf -profile slurm -w "${NEXTFLOW_WORK_DIR}" -process.echo -resume -with-trace --outdir "results/missing_samples"
    nextflow run main_missing_samples.nf -profile local -w "${NEXTFLOW_WORK_DIR}" -process.echo -resume -with-trace --outdir "results/missing_samples"
fi
